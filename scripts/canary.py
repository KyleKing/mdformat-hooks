...
settings too, under 'plugin.hooks'. That is the only way to canary a
plugin whose behavior is off by default::

    "options": {"plugin": {"hooks": {"argument": true}}}

'extensions' names other mdformat plugins the repo's own docs build depends on
(e.g. 'mkdocs' for MkDocs admonitions), on top of the 'hooks' this
